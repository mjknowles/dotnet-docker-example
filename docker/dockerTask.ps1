...
.PARAMETER Clean
Removes the image dotnet.docker.example and kills all containers based on that image.
...
Build a Docker image named dotnet.docker.example
#>
...
function Compose () {
    ...
    if (Test-Path $composeFileName) {
        Write-Host "Running compose file $composeFileName from $PSScriptRoot"
...
function StartDebugging () {
    ...
    docker exec -i $containerId bash -c 'dotnet restore; dotnet build' --interpreter=mi
